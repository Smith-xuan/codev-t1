...
# Source bashrc to ensure conda/micromamba is available (like generate_codev_ds32_vllm.sh)
...
# We need both to be available for environment switching
if command -v conda >/dev/null 2>&1; then
    # Initialize conda (for sandbox-runtime environment)
    eval "$(conda shell.bash hook 2>/dev/null)" || true
    echo "✓ Initialized conda (for sandbox-runtime environment)"
fi

if [ -f "/root/.local/bin/micromamba" ]; then
    # Initialize micromamba (for slime environment)
...
SANDBOX_PORT=${SANDBOX_PORT:-8181}  # SandboxFusion port (changed from IVERILOG_PORT)
...
cleanup_own_processes() {
    ...
    # Kill SandboxFusion server processes related to this script (on our specific port)
...
cleanup() {
    ...
    # Kill SandboxFusion server if it was started by this script
    ...
    # Also kill any SandboxFusion server processes on our port
...
# SANDBOX_PORT is already defined above
# Use the host IP for SANDBOX_URL (Ray workers will use this to access the server)
SANDBOX_URL="http://${HOST_IP}:${SANDBOX_PORT}/run_code"
# Also set IVERILOG_URL for backward compatibility (generate_with_iverilog.py will use SANDBOX_URL first)
IVERILOG_URL="${SANDBOX_URL}"

echo "Host IP for service access: ${HOST_IP}"
echo "SANDBOX_URL for Ray workers: ${SANDBOX_URL}"
echo "IVERILOG_URL (backward compatibility): ${IVERILOG_URL}"
...
# Check if SandboxFusion directory exists
...
if curl -s http://127.0.0.1:${SANDBOX_PORT}/health > /dev/null 2>&1 || \
   ...
   curl -s http://127.0.0.1:${SANDBOX_PORT}/run_code > /dev/null 2>&1; then
    ...
    # Start SandboxFusion server in background (using sandbox-runtime environment)
    echo "Starting SandboxFusion server on ${SANDBOX_HOST}:${SANDBOX_PORT}..."
    cd "$SANDBOX_DIR"
    
    # Activate sandbox-runtime environment and start server
    # sandbox-runtime is a conda environment (not micromamba)
    if command -v conda >/dev/null 2>&1; then
        ...
        echo "Please install conda or ensure conda is in PATH."
        exit 1
    fi
    ...
    # Switch back to slime environment (like generate_codev_ds32_vllm.sh does)
    # slime is a micromamba environment (not conda)
    # This ensures the rest of the script runs in the correct environment
    echo "Switching back to slime environment..."
    
    # First deactivate conda environment (sandbox-runtime) if active
    if command -v conda >/dev/null 2>&1; then
        conda deactivate 2>/dev/null || true
    fi
    
    # Then activate slime using micromamba
    if [ -f "/root/.local/bin/micromamba" ]; then
        # Deactivate any active micromamba environment first
        micromamba deactivate 2>/dev/null || true
        # Activate slime environment (micromamba)
        ...
        echo "Please ensure micromamba is available at /root/.local/bin/micromamba"
        exit 1
    fi
    
    # Wait for server to be ready (max 60 seconds, similar to verl's script)
...
CKPT_ARGS=(
   ...
   --ref-load /nfs_global/LLaMA-Factory/saves/qwen3-8b/full/tool_8.1k_ds32_10epochs/megatron
   --load /nfs_global/LLaMA-Factory/saves/qwen3-8b/full/tool_8.1k_ds32_10epochs/megatron_slime_save
   --save /nfs_global/LLaMA-Factory/saves/qwen3-8b/full/tool_8.1k_ds32_10epochs/megatron_slime_save
   --save-interval 10
)

# CKPT_ARGS=(
#    --hf-checkpoint ${MODEL_PATH}
#    --ref-load /nfs_global/LLaMA-Factory/saves/qwen3-1.7b/full/tool_8.1k_ds32_resummrized_10epochs/checkpoint-1270_torch_dist
#    --load /nfs_global/LLaMA-Factory/saves/qwen3-1.7b/full/tool_8.1k_ds32_resummrized_10epochs/checkpoint-1270_torch_dist
#    --save /nfs_global/LLaMA-Factory/saves/qwen3-1.7b/full/tool_8.1k_ds32_resummrized_10epochs/checkpoint-1270_torch_dist_slime_save
#    --save-interval 50
# )
...
ROLLOUT_ARGS=(
   ...
   --eval-max-response-len 30000  # Reduced for validation to ensure input + output <= 40960
...
PERF_ARGS=(
   ...
   --max-tokens-per-gpu 30000  # Matching verl's ppo_max_token_len_per_gpu
)
...
WANDB_ARGS=(
   ...
   --wandb-project verl_onpolicy_modified_format_reward_slime_test  # Matching verl's project_name
   --wandb-group codev_onpolicy_modified_format_reward_slime_test   # Matching verl's experiment_name
   --wandb-key 'e8f26cb646aea4a12ef982270212804afa4fa31e'
)

SGLANG_ARGS=(
   --rollout-num-gpus-per-engine 8  # Each engine uses 8 GPUs per node
   --sglang-mem-fraction-static 0.7  # Reduced to 0.6 to leave more memory for FSDP in colocate mode
...
# Verify Megatron-LM installation (check if megatron.core can be imported)
echo "Verifying Megatron-LM installation..."
if [ -d "/root/Megatron-LM" ] || [ -d "/nfs_global/Megatron-LM" ]; then
    python3 -c "import sys, os; megatron_path = '/root/Megatron-LM' if os.path.exists('/root/Megatron-LM') else '/nfs_global/Megatron-LM'; sys.path.insert(0, megatron_path); from megatron.core import mpu; print('  ✓ megatron.core imported successfully')" 2>/dev/null || {
        echo "  ⚠ Warning: megatron.core import failed on worker node"
        echo "  This may cause errors. Ensure Megatron-LM is properly installed."
        echo "  Trying to install Megatron-LM if it exists..."
        if [ -d "/root/Megatron-LM" ]; then
            cd /root/Megatron-LM && pip install -e . 2>/dev/null && echo "  ✓ Installed Megatron-LM" || echo "  ✗ Failed to install Megatron-LM"
        elif [ -d "/nfs_global/Megatron-LM" ]; then
            cd /nfs_global/Megatron-LM && pip install -e . 2>/dev/null && echo "  ✓ Installed Megatron-LM" || echo "  ✗ Failed to install Megatron-LM"
        fi
    }
else
    echo "  ⚠ Skipping megatron.core import check (Megatron-LM not found)"
fi
...
# SANDBOX_URL was already set before starting Ray using the host IP we determined
