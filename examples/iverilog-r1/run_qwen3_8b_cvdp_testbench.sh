...
export CVDP_PYTEST_PATH="/nfs_global/S/shiwenxuan/miniconda3/envs/slime/bin/pytest"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    # Resolve pytest path; || true prevents set -e from killing the script when
    # pytest is not on PATH.  Empty result is fine: cvdp_testbench_reward.py
    # falls back to `sys.executable -m pytest` when CVDP_PYTEST_PATH is unset/empty.
    CVDP_PYTEST_PATH="${CVDP_PYTEST_PATH:-$(which pytest 2>/dev/null || true)}"
