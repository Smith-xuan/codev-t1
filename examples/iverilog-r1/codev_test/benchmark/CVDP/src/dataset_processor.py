...
"""
    ...
    def run_subjective_scoring(self, id, repo, model=None, obj=False):
        ...
        if 'subjective_reference' in self.context[id]:
            ...
            reference = _FENCE_LINE_RE.sub("```", reference).strip()
        ...
        uut = None
        # print(f'Before `No content for subjective scoring`, id is {id}, cat is {cat}, self.files[id] is {self.files[id]}')
...
class CopilotProcessor (DatasetProcessor):
    ...
    def create_context (self, id : str, model = None):
        ...
        # print(f'id is {id}, self.golden is {self.golden}, context.keys are {context.keys()}')
        if self.golden:
        ...
        # ----------------------------------------
            ...
            if len(files) == 1 and 'direct_text' in output:
                ...
                for cxt in output['code']:
                    for filename, content in cxt.items():
                        result[filename] = content
                        parts.append(f"```\n{content}\n```")
                # 关键：给 BLEU/ROUGE 一个 uut
                result['subjective.txt'] = "\n".join(parts).strip()
            else:
            ...
            # print(f'id is {id}, use LLMs to evaluate. Model is {model} / {self.model}. Output is {output}. len(files) is {len(files)}, code in output is {"code" in output}.')
