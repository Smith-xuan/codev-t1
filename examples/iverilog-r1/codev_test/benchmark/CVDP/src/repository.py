...
class Repository:
    ...
    def restore_files(self, files):
        ...
        for file, content in files.items():
            ...
            # Fix cocotb version in Dockerfile to ensure cocotb.runner module is available
            if file == 'Dockerfile' or file.endswith('/Dockerfile'):
                content = self._fix_cocotb_version_in_dockerfile(content)
    ...
    def _fix_cocotb_version_in_dockerfile(self, content: str) -> str:
        """
        Fix cocotb version specification in Dockerfile to ensure cocotb.runner module is available.
        
        This function ensures that cocotb is installed with a version >= 1.6.0, which includes
        the cocotb.runner module required by some test harnesses.
        
        Args:
            content: Dockerfile content as string
            
        Returns:
            Updated Dockerfile content with cocotb version specified
        """
        import re
        
        # Required cocotb version (1.6.0+ includes cocotb.runner module)
        required_cocotb_version = ">=1.8.0"
        
        # Pattern 1: RUN pip install cocotb-bus (without cocotb)
        # Pattern 2: RUN pip3 install cocotb_bus (without cocotb)
        # We need to add cocotb with version before cocotb-bus/cocotb_bus
        
        # Check if there's already a cocotb installation line (with or without version)
        has_cocotb_install = re.search(r'RUN\s+(?:pip|pip3)\s+install.*\bcocotb\b(?!-bus|_bus)', content)
        
        # Check if there's cocotb-bus or cocotb_bus installation
        has_cocotb_bus = re.search(r'RUN\s+(?:pip|pip3)\s+install.*\b(cocotb-bus|cocotb_bus)\b', content)
        
        if has_cocotb_bus and not has_cocotb_install:
            # Replace: RUN pip install cocotb-bus
            # With: RUN pip install 'cocotb>=1.6.0' cocotb-bus
            pattern = r'(RUN\s+(?:pip|pip3)\s+install\s+)(cocotb-bus|cocotb_bus)'
            replacement = r'\1' + "'cocotb" + required_cocotb_version + "' " + r'\2'
            content = re.sub(pattern, replacement, content)
        elif has_cocotb_install:
            # If cocotb is already installed, ensure it has version requirement
            # Pattern: RUN pip install cocotb (without version)
            pattern = r'(RUN\s+(?:pip|pip3)\s+install\s+)(\bcocotb\b)(?!\s*[=<>!])'
            replacement = r'\1' + "'cocotb" + required_cocotb_version + "'"
            content = re.sub(pattern, replacement, content)
    ...
    def log_docker(self, docker : str = "", cmd : str = "", service : str = "", logfile : str = "", 
        ...
        with open(script_path, 'w') as script_file:
            ...
            # script_file.write(f"# Disable Docker BuildKit to avoid hostname resolution issues\n")
            # script_file.write(f"export DOCKER_BUILDKIT=0\n")
    ...
    def create_workspace_volume_script(self, docker_dir, repo_url=None, commit_hash=None, patches=None, root_dir=None):
        ...
        with open(create_script_path, 'w') as script_file:
            ...
            if repo_url and commit_hash:
                ...
                # script_file.write("  DOCKER_BUILDKIT=0 docker build -t patch_image -f \"$TEMP_DOCKERFILE\" .\n")
