...
## sample不重复在src/llm_lib/local_inference_model.py里面，不用手动筛，但机制应该是有点问题
...
def run_samples(args: argparse.Namespace, n_samples: int, k_threshold: int) -> None:
    ...
    for i in range(n_samples):
        ...
        if regenerate_only and i in existing_samples:
            ...
            # cp file to sample directory
            file_sample = os.path.join(sample_prefix, os.path.basename(args.prompts_responses_file))
            shutil.copyfile(args.prompts_responses_file, file_sample)
        ...
        for arg_name, arg_value in vars(args).items():
            ...
            if arg_name == 'prompts_responses_file':
                cmd.append("--prompts-responses-file")
                cmd.append(file_sample)
                continue
