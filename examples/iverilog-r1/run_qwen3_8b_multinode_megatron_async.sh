#!/bin/bash
# =============================================================================
# Fully-Async Multi-Node Training Script for Iverilog-R1 with Qwen3-8B
# =============================================================================
#
...
#
# Effect on long-tail samples:
#   The background worker always keeps rollout-batch-size tasks in flight.
#   Fast-finishing samples are collected immediately; slow/long-tail ones finish
#   later and their results feed the NEXT training step.  Training is never
#   blocked waiting for a single slow sample.
# =============================================================================
...
# GPUs (tp×cp×pp = 8×2×1 = 16), but we only allocate 8 to the actor.
# ---------------------------------------------------------------------------
ACTOR_NODES=1
...
WANDB_ARGS=(
   ...
   --wandb-group codev_onpolicy_modified_format_reward_slime_test_async
